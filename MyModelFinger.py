...
class MyModelFinger(MultiColorComponent):
    ...
    def __init__(self, parent, position, shaderProg, display_obj=None,
        ...
        self.componentList.append(first_finger)
        self.componentDict[f'{0}'] = first_finger
        ...
        if finger_nums > 1:
            ...
            for i in range(finger_nums - 1):
                ...
                self.componentList.append(curr_finger)
                self.componentDict[f'{i + 1}'] = curr_finger
                parent_finger = curr_finger

    def setPartRotateExtent(self, index, axis, min_deg: float = None, max_deg: float = None):
        curr_comp = self.componentList[index]
        if axis == 0:
