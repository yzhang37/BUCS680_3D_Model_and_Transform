...
class Cone(Shape):
    ...
    def __init__(self, position, shaderProg, scale, color: ColorType.ColorType = ColorType.YELLOW, lowPoly=False):
        """
        :param position: location of the object
        :type position: Point
        :param shaderProg: compiled shader program
        ...
        """
        if lowPoly:
            super(Cone, self).__init__(position, shaderProg, scale, self.verticesLP.copy(), self.indicesLP.copy(),
                                       color)
        else:
...
class Cube(Shape):
    ...
    def __init__(self, position: Point, shaderProg, scale, color: ColorType.ColorType = ColorType.RED):
        """
        :param position: location of the object
        :type position: Point
        :param shaderProg: compiled shader program
...
class Cylinder(Shape):
    ...
    def __init__(self, position, shaderProg, scale, color: ColorType.ColorType = ColorType.GREEN, lowPoly=False):
        """
        :param position: location of the object
        :type position: Point
        :param shaderProg: compiled shader program
        ...
        """
        if lowPoly:
            super(Cylinder, self).__init__(position, shaderProg, scale, self.verticesLP.copy(), self.indicesLP.copy(),
                                           color)
        else:
            super(Cylinder, self).__init__(position, shaderProg, scale, self.vertices.copy(), self.indices.copy(),
                                           color)
        # translate object by -z extent of the new component so that rotations occur @ the joint
        # rather than around the object's true center
        glutility = GLUtility.GLUtility()
